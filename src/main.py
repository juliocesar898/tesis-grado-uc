...
def main():
    ...
    logger.info("--- Iniciando Pipeline de DeepSignal (Clasificación de Modulaciones AMC) ---")
    ...
    try:
        ...
        if model is None:
            logger.warning("No se halló modelo preentrenado. Inicializando nuevo modelo y guardando...")
        ...
        logger.info(f"Clase predicha (índice): {predicted_class_idx}, Nivel de confianza: {confidence:.2%}")
        logger.info("----------------------------------")
    except Exception as e:
